# 🏏 IPL Analysis Dashboard (2008 - 2025)

An adaptive and interactive Power BI dashboard designed to analyze IPL seasons from 2008 to 2025 using dynamic filters, KPI metrics, and sports analytics visualizations.

---

## 📷 Dashboard Preview
-![Dashboard Preview](https://github.com/Tushar-Pillay/Data-Analytics-Portfolio/blob/main/PowerBI/IPL%20Analysis%20Dashboard%20(2008%20-%202025)/Dashboard%20Preview.png)

---

## 🔗 Reference Links 

- [Powerbi File](https://github.com/Tushar-Pillay/Data-Analytics-Portfolio/blob/main/PowerBI/IPL%20Analysis%20Dashboard%20(2008%20-%202025)/IPL%20Analysis.pbix)

- [Kaggle IPL Dataset](https://www.kaggle.com/datasets/chaitu20/ipl-dataset2008-2025)

- [Uploded Dataset](https://github.com/Tushar-Pillay/Data-Analytics-Portfolio/tree/main/PowerBI/IPL%20Analysis%20Dashboard%20(2008%20-%202025)/Dataset)

- [IPL Official](https://www.iplt20.com/)

---

## 📌 Project Overview

This dashboard allows users to dynamically select any IPL season and instantly explore:
- Team Performance
- Points Table Analysis
- Orange Cap & Purple Cap Statistics
- Boundary Analysis
- Season KPIs
- Winner & Runner-Up Details

The project demonstrates interactive reporting, data storytelling, and business intelligence using Power BI.

---

## 🚀 Dashboard Features

### ✅ Dynamic Season Filter
- Select any IPL season from 2008–2025
- Entire dashboard updates dynamically

### ✅ KPI Metrics
- Total Matches
- Total Venues
- Total Teams
- Total 4's
- Total 6's
- Half Centuries
- Centuries

### ✅ Season Summary
- Season Winner
- Runner-Up Team
- Dynamic Team Logos

### ✅ Orange Cap Analysis
- Orange Cap Holder
- Total Runs
- Team Name
- Dynamic Player Image

### ✅ Purple Cap Analysis
- Purple Cap Holder
- Total Wickets
- Team Name
- Dynamic Player Image

### ✅ Boundary Analysis

#### Top 4's Analysis
- Player Name
- Total 4's
- Team Name

#### Top 6's Analysis
- Player Name
- Total 6's
- Team Name

### ✅ Dynamic Points Table
- Team Logo
- Team Name
- Matches Played
- Won
- Lost
- No Result
- Tie
- Total Points

---

## 🛠️ Tools & Technologies Used

- Power BI
- DAX
- Power Query
- Data Modeling
- Interactive Visualizations
- KPI Reporting
- Sports Analytics

---

## 📊 Dashboard Highlights

- Adaptive Dashboard Layout
- Interactive Slicers & Filters
- Dynamic Images & Logos
- Responsive KPI Cards
- Sports Analytics Visualization
- Business-Oriented Dashboard Design




